...
public class ForestController {
    ...
    @FXML private VBox vboxForest;
    // Selected nodes variables
    private Set<TreeNode> selectedNodes = new HashSet<>();
    ...
    @FXML private HBox BoxUnassignedNodes;
    @FXML private HBox MainTreePane;
    @FXML private StackPane stackPane;
    @FXML private Pane topPane;
    @FXML private VBox vboxNotifications;
    @FXML private Button ButtonRotateR;
    @FXML private Button ButtonRotateL;
    @FXML private ToggleButton ToggleConnecting;
    @FXML private ToggleButton ToggleMove;
    @FXML private CheckBox ShowPlayersCheckBox;
    @FXML private CheckBox ShowOutcomesCheckBox;
    @FXML private Button btnCheckConstraints;
    ...
    public DoubleProperty scaleProperty(){
        return this.mainController.scaleProperty();
    }
    ...
    @FXML private void toggleConnecting(){
        toggleGroup.selectToggle(ToggleConnecting);
    }

    @FXML private void toggleMove(){
        toggleGroup.selectToggle(ToggleMove);
    }

    @FXML void rotateLeft(){
    ...
    private void newTreePosition(TreePane pane){
        ...
        setSelectedNode(pane.getRootNode());
    }

    void addTreeNode(TreeNode root){
        TreePane treePane = new TreePane(this,root);
    ...
    private void updateSelectedNodes(){
        mainController.updateSelectedNodes(selectedNodes);
    }

    private void cleanSelectedNodes() {
        for (TreeNode c: this.selectedNodes) {
            c.setSelected(false);
        }
        this.selectedNodes.clear();
    }

    public void setSelectedNode(TreeNode shape) {
        cleanSelectedNodes();
        shape.setSelected(true);
        this.selectedNodes.add(shape);
        updateSelectedNodes();
    }

    public void toggleSelectedNode(TreeNode shape) {
        if (this.selectedNodes.contains(shape)){
    ...
    @FXML private void checkConstraints(){
        ...
        if (tree.getRoot() instanceof Outcome){
            this.vboxNotifications.getChildren().add(new NotificationBox("Tree only consists of an outcome!", vboxNotifications));
        } else {
            ...
            if (!hasInformationSet){
                this.vboxNotifications.getChildren().add(new NotificationBox("Not all nodes have an informationset assigned!", vboxNotifications));
            }
            if (!hasNoNodesAsLeaf){
                this.vboxNotifications.getChildren().add(new NotificationBox("Nodes cannot be leaves!", vboxNotifications));
            }
            if (!informationsetsNotEqualActions.isEmpty()){
                String out = Arrays.toString(informationsetsNotEqualActions.toArray());
                this.vboxNotifications.getChildren().add(new NotificationBox("The following informationsets don't have the same amount of actions: "
            ...
            if (!informationsetsNoPlayer.isEmpty()){
                ...
                this.vboxNotifications.getChildren().add(new NotificationBox("The following informationsets don't have a player assigned: "
                        + out.substring(1, out.length()-1) + ".", vboxNotifications));
            }
            if (!informationsetsNotCanonical.isEmpty()){
                String out = Arrays.toString(informationsetsNotCanonical.toArray());
                this.vboxNotifications.getChildren().add(new NotificationBox("The following informationsets have nodes which cover the same subtree (not canonical): "
                        + out.substring(1, out.length()-1) + ".", vboxNotifications));
            }
        }
    }
